...
void logDebug(Rc5 *dev, uint8_t type)
{
  ...
  dev->debugEvents[dev->debugCnt].timtime = __HAL_TIM_GET_COUNTER(dev->init.tim);
...
void rc5_handleFalling(Rc5 *dev) {
  ...
  if (dev->irqStage == RC5_IRQ_WAIT_S1  || ( (HAL_GetTick() - dev->watchdog) > RC5_WATCHDOG_MS)) {
...
void rc5_handleTimEvent(Rc5 *dev) {
  ...
  int32_t deltaSync = __HAL_TIM_GET_AUTORELOAD(dev->init.tim) - dev->syncValue;
  if( abs((int)deltaSync) < dev->halfBitDuration )
  {
    __HAL_TIM_SET_AUTORELOAD(dev->init.tim, dev->fullBitDuration + (dev->halfBitDuration/2.0f - deltaSync));
  }
  else {
    __HAL_TIM_SET_AUTORELOAD(dev->init.tim, dev->fullBitDuration + (dev->halfBitDuration*3.0f/2.0f - deltaSync));
  }
  ...
  if (dev->bitCounter == 0) {
    ...
    dev->pkt.command |=
        (bit & 0x1) << (CMD_BIT_SIZE - (dev->bitCounter - CMD_BIT_OFFSET) - 1);
