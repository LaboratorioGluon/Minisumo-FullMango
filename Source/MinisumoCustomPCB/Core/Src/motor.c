...
void motor_setDuty(Motor *dev, MotorDirection dir, uint32_t duty) 
{
    ...
    {
        HAL_GPIO_WritePin(dev->init.directionPort, dev->init.directionPin, GPIO_PIN_RESET);
    }
    else {
        localDuty = 100-duty;
        HAL_GPIO_WritePin(dev->init.directionPort, dev->init.directionPin, GPIO_PIN_SET);
    }
