                   ...
                   uint32_t zerosCount)
{
    for (uint32_t led = 0; led < numLeds; led++) {
        ...
        for (uint32_t bit = 0; bit < 8; bit++) {
            ...
            wsData[baseOffset + bit] =
                (color.g & (0x80 >> bit)) ? ws->one.timeH : ws->zero.timeH;
            wsData[baseOffset + bit + 8] =
                (color.r & (0x80 >> bit)) ? ws->one.timeH : ws->zero.timeH;
            wsData[baseOffset + bit + 16] =
                (color.b & (0x80 >> bit)) ? ws->one.timeH : ws->zero.timeH;
        }
    }

    for (uint32_t i = 0; i < zerosCount; i++) {
        wsData[numLeds * 24U + i] = 0x0;
    }
}