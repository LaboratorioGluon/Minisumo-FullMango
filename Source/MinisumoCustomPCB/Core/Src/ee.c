...
#error EE Library should be set manually for your MCU!
        ...
        ee_handle.address = (FLASH_EDATA_BASE_NS + ee_handle.page_sector_size * (ee_handle.page_sector_number * 2 + 1));
...
void ee_read(void)
{
    ...
    if (data != NULL) {
        uint32_t address = ee_handle.address;
        uint32_t words   = (ee_handle.size + 3) / 4;  // Redondear a palabras de 32 bits

        for (uint32_t i = 0; i < words; i++) {
            // Leer la palabra completa de 32 bits desde EDATA
            uint32_t reg_val = *(__IO uint32_t*)(address + (i * 4));

            // Copiar los bytes correspondientes al buffer de salida
            for (uint32_t b = 0; b < 4; b++) {
                if ((i * 4 + b) < ee_handle.size) {
                    *data++ = (uint8_t)(reg_val >> (b * 8));
                }
            }
        }
    }
...
bool ee_write(void)
{
    ...
    do {
        ...
        data             = ee_handle.data;
        uint32_t address = ee_handle.address;
        uint32_t words   = (ee_handle.size + 3) / 4;  // Cantidad de palabras de 32 bits

        for (uint32_t i = 0; i < words; i++) {

            uint32_t flash_val = *(__IO uint32_t*)(address + (i * 4));

            for (uint32_t b = 0; b < 4; b++) {
                uint32_t byte_idx = (i * 4) + b;

                if (byte_idx < ee_handle.size) {
                    uint8_t flash_byte = (uint8_t)(flash_val >> (b * 8));
                    if (data[byte_idx] != flash_byte) {
                        answer = false;
                        break;
                    }
                }
